skovgaard=function(model1,model2){
  ...
  J11=as.matrix(
    Jtilde[!c(nomesx1%in%nomesx2,nomesz1%in%nomesz2),
           !c(nomesx1%in%nomesx2,nomesz1%in%nomesz2)]
  )
  V1=varya
  MUhat=diag(mu,n)
  MUtilde=diag(mu2,n)
  ...
  epsb=(abs(det(UP))*sqrt(abs(det(as.matrix((Ktilde%*%solve(UP)%*%Jhat%*%K_1hat%*%UP)[!c(nomesx1%in%nomesx2,nomesz1%in%nomesz2),!c(nomesx1%in%nomesx2,nomesz1%in%nomesz2)])))))
